...
_bench_init :
	...
	$(eval opts = $(base_opts) $(mcs_opts)              ) $(eval suf = $(base_suf)$(mcs_suf))
	...
	make _run_gp_ds1 exp_name=$(exp_name) opts="$(opts)" suf=$(suf)
	$(eval opts = $(base_opts) $(mcs_opts) -uiw        ) $(eval suf = $(base_suf)$(mcs_suf).iw)
	make _run_gp_ds1 exp_name=$(exp_name) opts="$(opts)" suf=$(suf)
	$(eval opts = $(base_opts) $(mcs_opts) -uiw -loo   )  $(eval suf = $(base_suf)$(mcs_suf).iw.loo)  # VIMCO
	make _run_gp_ds1 exp_name=$(exp_name) opts="$(opts)" suf=$(suf)
	...
	$(eval opts = $(base_opts) $(mcs_opts) -uiw        ) $(eval suf = $(base_suf)$(mcs_suf).iw)
	make _run_gp_ds1 exp_name=$(exp_name) opts="$(opts)" suf=$(suf)
	$(eval opts = $(base_opts) $(mcs_opts) -uiw -loo   )  $(eval suf = $(base_suf)$(mcs_suf).iw.loo)
	make _run_gp_ds1 exp_name=$(exp_name) opts="$(opts)" suf=$(suf)
	...
	$(foreach seed, $(seeds), \
		$(foreach emb_dim, $(emb_dims), \
			$(foreach emb_qscale, $(emb_qscales), \
				...
				make _bench_init seed=$(seed) base_opts="$(_base_opts)" base_suf=$(_base_suf); \
			)))

bench_init_all:
	$(eval base_opts = -ua) $(eval base_suf = .ua)
	$(eval _base_opts = $(base_opts) -es euclid  -edt diag ) $(eval _base_suf = $(base_suf).n)
	make bench_init_all1 base_opts="$(_base_opts)" base_suf=$(_base_suf)
