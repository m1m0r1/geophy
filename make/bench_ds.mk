...
_bench_ds :
	...
	make _run_gp_$(ds) exp_name=$(exp_name) opts="$(opts)" suf=$(suf)
...
bench_ds_all1:
	...
	$(eval dss = ds1 ds2 ds3 ds4 ds5 ds6 ds7 ds8)
	$(eval emb_dims = 2 4)
		...
		$(foreach seed, $(seeds), \
			$(foreach emb_dim, $(emb_dims), \
				$(foreach emb_qscale, $(emb_qscales), \
					...
					make _bench_ds ds=$(ds) seed=$(seed) base_opts="$(_base_opts)" base_suf=$(_base_suf); \
				))))

bench_ds_all:
	$(eval base_opts = -ua) $(eval base_suf = .ua)
	$(eval _base_opts = $(base_opts) -es euclid  -edt diag ) $(eval _base_suf = $(base_suf).n)
	make bench_ds_all1 base_opts="$(_base_opts)" base_suf=$(_base_suf) emb_dims="2 4"
